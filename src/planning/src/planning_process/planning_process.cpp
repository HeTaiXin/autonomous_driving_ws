...
namespace Planning
{
    PlanningProcess::PlanningProcess() : Node("planning_process") // 规划总流程
    {
        ...
        obs_dis_ = process_config_->get_process().obs_dis_;

        // 创建主车和障碍物
        //   会执行MainCarInfo()初始化函数, 获得主车信息
        car_ = std::make_shared<MainCarInfo>();
        for (int i = 0; i < 3; i++)
        {
            auto obs_car_ = std::make_shared<ObsCarInfo>(i + 1);
    ...
    void PlanningProcess::vehicle_spawn(const std::shared_ptr<VehicleInfoBase> &vehicle) // 生成车辆
    {
        ...
        spawn.header.frame_id = process_config_->get_pnc_map().frame_;
        spawn.child_frame_id = vehicle->get_child_frame();
        ...
        tf_broadcaster_->sendTransform(spawn); // 发送广播内容
    }

    void PlanningProcess::get_location(const std::shared_ptr<VehicleInfoBase> &vehicle) // 监听定位点
    {
        try
        {
            PoseStamped point;
            TransformStamped ts_result;
            ts_result = buffer_->lookupTransform(
                process_config_->get_pnc_map().frame_, // 父坐标系
                vehicle->get_child_frame(),            // 子坐标系
                tf2::TimePointZero                     // 在当前时间接收广播内容
            );
            point.pose.position.x = ts_result.transform.translation.x;
            point.pose.position.y = ts_result.transform.translation.y;
            point.pose.position.z = ts_result.transform.translation.z;
            point.pose.orientation.x = ts_result.transform.rotation.x;
            point.pose.orientation.y = ts_result.transform.rotation.y;
            point.pose.orientation.z = ts_result.transform.rotation.z;
            point.pose.orientation.w = ts_result.transform.rotation.w;

            vehicle->updata_location(point); // 位置参数更新
        }
    ...
    template <typename T>
    bool PlanningProcess::connect_server(const T &client) // 链接服务器
    {
        // 判断客户端类型
        ...
        // 等待服务器
        // 阻塞等待最多 1 秒，看服务是否上线。如果没上线就返回 false，然后while循环继续等。
        ...
        RCLCPP_INFO(this->get_logger(), "%s server connected successfully!", server_name.c_str());
        return true;
    }
    ...
    bool PlanningProcess::map_request() // 发送地图请求
    {
        ...
        request->map_type = process_config_->get_pnc_map().type_;
    ...
    bool PlanningProcess::global_path_request() // 发送全局路径请求
    {
        ...
        request->global_planner_type = process_config_->get_global_path().type_;
        ...
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), result_future) == rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            global_path_ = result_future.get()->global_path; // 获取响应中的global_path
            return true;
        }
        else
        {
    ...
    void PlanningProcess::planning_callback() // 总流程的回调函数
    {
        ...
        // 参考线
        const Referline refer_line = refer_line_creator_->create_reference_line(global_path_, car_->get_loc_point());
        if (refer_line.refer_line.empty())
        {
            RCLCPP_ERROR(this->get_logger(), "reference line empty!");
            return;
        }
        const Path refer_line_rviz = refer_line_creator_->referline_to_rviz(); // 生成rviz用的参考线
        refer_line_pub_->publish(refer_line_rviz);                             // 发布供rviz订阅的参考线
        ...
        RCLCPP_INFO(this->get_logger(), "--------- car state: loc: (%.2f, %.2f), speed: (%.2f, %.2f), theta: %.2f, kappa: %.2f",
                    car_->get_loc_point().pose.position.x,
                    car_->get_loc_point().pose.position.y,
                    car_->get_speed(), car_->get_acceleration(),
                    car_->get_theta(), car_->get_kappa());

        const rclcpp::Time end_time = this->get_clock()->now();
        double const planning_total_time = end_time.seconds() - start_time.seconds();
