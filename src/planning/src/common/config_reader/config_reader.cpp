...
namespace Planning
{
    ConfigReader::ConfigReader()  // 配置文件读取器
    {
        ...
        // 获取配置文件
        try {
            planning_config = YAML::LoadFile(planning_share_directory 
                + "/config/planning_static_obs_config.yaml");
        }catch (const YAML::BadFile &e) {
            RCLCPP_ERROR(rclcpp::get_logger("config"), "Failed to load config: %s", e.what());
        }
    ...
    void ConfigReader::read_local_speeds_config()
    {
        ...
        {
            read_pnc_map_config();
            local_speed_.speed_size_ = planning_config["local_speed"]["speed_size"].as<int>();
        }
        catch (const YAML::Exception &e)
        {
