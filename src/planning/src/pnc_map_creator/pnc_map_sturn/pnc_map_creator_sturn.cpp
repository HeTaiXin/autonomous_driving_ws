...
namespace Planning
{
    PNCMapCreatorSTurn::PNCMapCreatorSTurn() // S弯道地图
    {
        ...
        // 地图起点坐标
        p_mid_.x = -3.0;
        p_mid_.y = pnc_map_config_->get_pnc_map().road_half_width_ / 2.0;

        // 长度和角度步长
        len_step_ = pnc_map_config_->get_pnc_map().segment_len_;
        theta_step_ = 0.01;
    ...
    void PNCMapCreatorSTurn::creat_pnc_map() // 生成地图
    {
        draw_straight_x(pnc_map_config_->get_pnc_map().road_length_ / 3.0, 1.0);
        draw_arc(M_PI_2, 1.0);
        draw_arc(M_PI_2, -1.0);

        // 保证pnc_map_.midline.points为偶数，否则rviz显示不出来
    ...
    {
        pnc_map_.header.frame_id = pnc_map_config_->get_pnc_map().frame_;
        pnc_map_.header.stamp = rclcpp::Clock().now();
        pnc_map_.road_half_width = pnc_map_config_->get_pnc_map().road_half_width_;
        ...
        pnc_map_.midline.lifetime = rclcpp::Duration::max(); // 在rviz显示的生命周期
        pnc_map_.midline.frame_locked = true; // 中心线锁定
    ...
    void PNCMapCreatorSTurn::draw_straight_x(const double &length, const double &plus_flag, const double &ratio)
    {
        ...
        {
            pl_.x = p_mid_.x;
            pl_.y = p_mid_.y + pnc_map_config_->get_pnc_map().road_half_width_;
            pr_.x = p_mid_.x;
            pr_.y = p_mid_.y - pnc_map_config_->get_pnc_map().road_half_width_;
    ...
    void PNCMapCreatorSTurn::draw_arc(const double &angle, const double &plus_flag, const double &ratio)
    {
        ...
        {
            pl_.x = p_mid_.x - pnc_map_config_->get_pnc_map().road_half_width_ * std::sin(theta_current_);
            pl_.y = p_mid_.y + pnc_map_config_->get_pnc_map().road_half_width_ * std::cos(theta_current_);
            pr_.x = p_mid_.x + pnc_map_config_->get_pnc_map().road_half_width_ * std::sin(theta_current_);
            pr_.y = p_mid_.y - pnc_map_config_->get_pnc_map().road_half_width_ * std::cos(theta_current_);
